//
//  MESHBeaconDefinitions.h
//  MeshSDK
//
//  Version : 1.0.0
//  Created by MTC on 19/02/18.
...
//针对BleSupportsExtension设备，请直接使用功率数值。
#define DEFAULT_TX_EX  0

#define DEFAULT_NAME  @"MeshBeacon"
...
typedef NS_OPTIONS(NSUInteger, BleSupports) {
    BleSupportsCC254x                   = 1 << 0,//TI系列
    BleSupportsNordic                   = 1 << 1,//Nordic系列
    BleSupportsLight                    = 1 << 2,//光感支持
    BleSupportsCombineCharacteristic    = 1 << 3,//组合特征
    BleSupportsAntiLose                 = 1 << 4,//已废弃，防丢器
    BleSupports16Key                    = 1 << 5,//加密模式
    BleSupportsUpdateName               = 1 << 6,//设备名轮播
    BleSupportsAli                      = 1 << 7,//已废弃，替换为eddystone
    BleSupportsEddystone                = BleSupportsAli,//Google Eddystone协议
    BleSupportsSerialData               = 1 << 8,//串口数据收发
    BleSupportsAdvRFOff                 = 1 << 9,//广播频点配置
    BleSupportsUserData                 = 1 << 10,//广播自定义数据
    BleSupportsExtension                = 1 << 11,//自定义扩展
    BleSupportsEncrypt                  = 1 << 12,//广播加密
    BleSupportsAB                       = 1 << 13//AB芯片
};
...
typedef NS_ENUM(NSInteger,ErrorCode)
{
    ...
    ErrorCode101 = 101,     //未识别的设备(未检测到peripheral或非MeshBeacon设备)
...
typedef void(^MESHCompletionBlock)(BOOL complete, NSError* error);
typedef void (^MESHDataCompletionBlock)(id data,NSError *error);


////////////////////////////////////////////////////////////////////
// Interface definition

@interface MESHBeaconDefinitions : NSObject
