...
#import "MESHBeaconDefinitions.h"

@class MESHBeacon;

////////////////////////////////////////////////////////////////////
// MTC delegate protocol


/**
 
 MESHBeaconDelegate 定义了 beacon 连接相关的委托方法，beacon 的连接是一个异步操作，因此你只需要实现例如beaconDidDisconnect:相关方法，它们会被自动回调.
 
 */

@protocol MESHBeaconDelegate <NSObject>
 ...
 *
 */
- (void)beaconConnection:(MESHBeacon*)beacon withError:(NSError*)error;
 ...
 *
 */
- (void)beaconDidDisconnect:(MESHBeacon*)beacon withError:(NSError*)error;
 ...
 *
 */
- (void)beacon:(MESHBeacon *)beacon didUpdateValue:(NSData *)value error:(NSError *)error;
...
/**
 ...
 MESHBeacon类代表了一个在该区域监听中遇到的beacon设备. 你不需要直接实例化这个类. MESHBeaconManager在扫描到beacon设备时会回调相应delegate的方法传人MESHBeacon.你可以通过MESHBeacon里的信息来识别不同的设备.
 
 
 MESHBeacon类既包含了苹果CLBeacon类的基本方法，同时也新增了一些扩展方法，它能连接到beacon设备读取和写人一些特征（characteristics）.
 
 */

@interface MESHBeacon : NSObject <CBPeripheralDelegate>

@property (nonatomic, weak)     id <MESHBeaconDelegate>  delegate;
 ...
 *
 *  Discussion:
 *    beacon设备的物理地址.可以唯一标识MTC设备，非MeshBeacon设备该值可能为空
 */
 ...
 *    是否是MeshBeacon
 */
@property (nonatomic, assign)    BOOL    isMeshBeacon;
 ...
 */
- (void)sendBeaconValue:(NSData *)value withCompletion:(MESHDataCompletionBlock)completion;
...
/**
 * 重置beacon设备默认KEY，该操作可以解除设备开发者绑定，让beacon设备可以重新被连接设定新的APP_KEY，该操作要求已经成功执行[MESHBeaconManager registerApp:APP_KEY];
 *
 */
- (void)resetSDKKEY;
